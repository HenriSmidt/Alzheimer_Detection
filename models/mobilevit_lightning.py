...
class MobileViTLightning(pl.LightningModule):
    def __init__(
        ...
        self_distillation_temperature=2.0,
    ):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if soft_labels is not None:
            ...
            logits_distilled = torch.nn.functional.log_softmax(
                logits / self.self_distillation_temperature, dim=1
            )
            soft_labels_distilled = torch.nn.functional.softmax(
                soft_labels / self.self_distillation_temperature, dim=1
            )
            # Compute knowledge distillation loss
            loss_kd = self.kd_criterion(logits_distilled, soft_labels_distilled)
            # Combine the two losses
            loss = self.self_distillation_alpha * loss_ce + (
                1 - self.self_distillation_alpha
            ) * loss_kd * (self.self_distillation_temperature**2)
        else:
            loss = loss_ce
