...
# Function to process CSV files and calculate F1 scores
def process_csv_files(category, model_name, slice_number=None):
    f1_scores[f"{category}_{model_name}"] = {class_label: [] for class_label in class_labels}

    if slice_number is not None:
        f1_scores_single_slice[f"{category}_{model_name}"] = {class_label: [] for class_label in class_labels}
    ...
    for csv_file in os.listdir(model_path):
        ...
        for i, class_label in enumerate(class_labels):
            f1_scores[f"{category}_{model_name}"][class_label].append(class_f1_scores[i])

        if slice_number is not None:
            slice_preds_single = np.array([softmax(np.array(ast.literal_eval(pred))) for pred in df[f"slice_{slice_number}"]])
            y_pred_single = np.argmax(slice_preds_single, axis=1)
            class_f1_scores_single = f1_score(y_true, y_pred_single, average=None, labels=class_labels)
            for i, class_label in enumerate(class_labels):
                f1_scores_single_slice[f"{category}_{model_name}"][class_label].append(class_f1_scores_single[i])
...
def plot_data(ax, data, labels, title, ylabel=None):
    ...
    for i in range(len(labels)):
        positions.extend(range(i * len(custom_class_labels) + 1, (i + 1) * len(custom_class_labels) + 1))
    ...
    for idx in range(len(labels)):
        for class_label in class_labels:
            ...
            box = ax.boxplot(class_data, positions=[positions[pos_offset]], widths=0.6, patch_artist=True)
            for patch in box['boxes']:
                patch.set_facecolor('lightblue')

            means = [np.mean(d) for d in [class_data]]
            for i, mean in enumerate(means):
                ax.text(positions[pos_offset], mean, f'{mean:.2f}', ha='center', va='bottom', color='red', fontsize=12)
    ...
    strategy_positions = [np.mean(flattened_positions[i * len(custom_class_labels):(i + 1) * len(custom_class_labels)]) for i in range(len(labels))]
    ...
    secax.tick_params(axis='x', which='both', length=0)  # Hide the secondary x-axis ticks
    secax.set_xlabel("CDR per Strategy")

    ax.set_ylim(y_min, y_max)

# Plot EfficientNet models
plot_data(axes[0], efficientnet_data, efficientnet_labels, "EfficientNet-b2 Models", ylabel="F1 Score")

# Plot MobileViT models
plot_data(axes[1], mobilevit_data, mobilevit_labels, "MobileViT-s Models")

plt.tight_layout()
plt.savefig(f"plots/detailed_custom_comparative_f1_scores_boxplot.pdf", format="pdf", bbox_inches="tight")
plt.show()
plt.close()
