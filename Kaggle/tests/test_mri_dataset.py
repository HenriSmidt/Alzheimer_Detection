...
from dataset import MRIDataset, MRIDataModule  # Ensure your classes are correctly imported
...
class TestMRIDataWithRealData(unittest.TestCase):
    ...
    def test_subject_leakage(self):
        ...
        self.assertTrue(train_subjects.isdisjoint(test_subjects), "Subjects should not overlap between train and test sets")
    ...
    def test_complete_session_load(self):
        ...
        for dataset in [self.data_module.train_dataset, self.data_module.test_dataset]:
            for index in range(len(dataset.data)):
                ...
                actual_slices = [int(path.split('_')[-1].split('.')[0]) for path in sample['paths']]
                self.assertEqual(expected_slices, actual_slices, "Not all slices from a session are loaded")
